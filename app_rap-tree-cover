/*
App for exploring RAP tree cover at native 30m resolution.
...
var buildPanel = function(map, position) {
  ...
  var updateMap = function() {
    ...
    var belowThreshold = rap.lt(vals.maxTree);
    var treeCov = rap.updateMask(mask).updateMask(belowThreshold);
    var grayArea = ee.Image(1).updateMask(mask).updateMask(belowThreshold.not());

    var viz = {min: 0, max: vals.colorMax, palette: ['white', 'green']};
    map.layers().reset([
      ui.Map.Layer(grayArea, {palette: ['gray']}, 'tree cover >= ' + vals.maxTree + '%'),
      ui.Map.Layer(treeCov, viz, 'tree cover (%)')
    ]);
  };
  ...
  var panel = ui.Panel({
    widgets: [
      ui.Label('RAP tree cover ' + yearStart + '–' + yearEnd,
        {fontWeight: 'bold', fontSize: '12px'}),
      makeSlider('Show areas with tree cover < (%; otherwise gray)', 0, 100, 1, defaults.maxTree, 'maxTree'),
...
var infoPanel = ui.Panel({
  widgets: [ui.Label(
    'RAP v3 tree cover (30m)\n' + yearStart + '–' + yearEnd + ' mean',
